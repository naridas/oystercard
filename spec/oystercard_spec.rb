...
describe Oystercard do
  ...
  context 'touch_in/touch_out' do
	...
	context '#touch_out' do
		it "can touch out" do
  		oyster_topped.touch_out(exit_station)
  		expect(oyster_topped).not_to be_in_journey
		end
		it 'raises an error if the £1 not on card' do
  		expect{ oyster_topped.touch_out(exit_station)}.to change{ oyster_topped.balance }.by -minimum_fare
		end
  ...
  context 'complete journeys' do
    it 'add the journey to journeys' do
      oyster_topped.touch_out(exit_station)
      journey = {}
      journey[station] = exit_station
      expect(oyster_topped.journeys).to include journey
    end
	end
end
